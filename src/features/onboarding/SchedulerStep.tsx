                                                    ...
                                                    >
                                                        {cell ? (
                                                            <div className="flex flex-col gap-4 h-full animate-in fade-in zoom-in-95 duration-500 relative z-10 text-left">
                                                                {/* Selector de Grado con Ciclo Automático */}
                                                                <div className="grid grid-cols-2 gap-2">
                                                                    <div className="relative text-left">
                                                                        <label 
                                                                            style={theme ? { color: theme.accent, opacity: 0.6 } : {}}
                                                                            className="text-[7px] font-black uppercase tracking-[0.2em] text-brand-magenta/60 mb-1.5 block px-1"
                                                                        >
                                                                            Ciclo CNEB
                                                                        </label>
                                                                        <div 
                                                                            style={theme ? { color: theme.accent, borderColor: `hsla(${theme.hue}, 80%, 50%, 0.2)`, backgroundColor: `hsla(${theme.hue}, 80%, 50%, 0.05)` } : {}}
                                                                            className="bg-brand-magenta/5 text-[10px] font-black text-brand-magenta px-3 py-2.5 rounded-xl border border-brand-magenta/20 italic flex items-center h-[35px]"
                                                                        >
                                                                            Ciclo {cell.ciclo}
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div className="relative group/sel text-left">
                                                                        ...
                                                                        <select
                                                                            ...
                                                                            className="appearance-none bg-white/5 text-[10px] font-black text-white px-3 py-2.5 rounded-xl border border-white/10 focus:border-brand-magenta/40 outline-none cursor-pointer hover:bg-white/10 transition-all w-full pr-7"
                                                                        >
                                                                            {allGradosForNivel.map(g => (
                                                                ...
                                                                {/* Área Académica - DISEÑO PREMIUM NO TRUNCADO */}
                                                                <div className="space-y-1.5 flex-1 flex flex-col justify-end text-left">
                                                                    <label 
                                                                        style={theme ? { color: theme.accent, opacity: 0.6 } : {}}
                                                                        className="text-[7px] font-black uppercase tracking-[0.2em] text-brand-magenta/60 block px-1"
                                                                    >
                                                                        Asignatura / Área
                                                                    </label>
                                                                    <div className="relative group/area">
                                                                        <div 
                                                                            style={theme ? { background: `linear-gradient(to right, hsla(${theme.hue}, 80%, 50%, 0.2), transparent)` } : {}}
                                                                            className="absolute -inset-0.5 bg-gradient-to-r from-brand-magenta/20 to-transparent blur opacity-0 group-hover/area:opacity-100 transition-opacity rounded-2xl" 
                                                                        />
                                                                        <select
                                                                            value={cell.area}
                                                                            onChange={(e) => updateCellData(rowIndex, day, { area: e.target.value })}
                                                                            style={theme ? { 
                                                                                borderColor: `hsla(${theme.hue}, 80%, 50%, 0.3)`,
                                                                                backgroundColor: `hsla(${theme.hue}, 80%, 50%, 0.1)`,
                                                                            } : {}}
                                                                            className="relative appearance-none bg-brand-magenta/10 hover:bg-brand-magenta/20 text-[10px] font-bold text-white px-4 py-3 rounded-2xl border border-brand-magenta/20 focus:border-brand-magenta outline-none cursor-pointer transition-all w-full pr-10 min-h-[50px] leading-tight flex items-center shadow-lg"
                                                                        >
                                                                            {nivelConfig.areas.map(a => (
                                                                                <option key={a} value={a} className="bg-[#111] text-white whitespace-normal py-2">{a}</option>
                                                                            ))}
                                                                        </select>
                                                                        <span 
                                                                            style={theme ? { color: theme.accent } : {}}
                                                                            className="material-icons-round absolute right-3 top-1/2 -translate-y-1/2 text-xl text-brand-magenta group-hover:scale-110 transition-transform"
                                                                        >
                                                                            expand_more
                                                                        </span>
                                                                    </div>
                                                                ...
                                                                {/* Kill Switch (Rediseñado) */}
                                                                <button
                                                                    onClick={(e) => { e.stopPropagation(); clearCell(rowIndex, day); }}
                                                                    className="absolute -top-2 -right-2 w-8 h-8 bg-[#1a1a1a] text-white/40 rounded-full flex items-center justify-center opacity-0 group-hover/cell:opacity-100 hover:bg-red-500 hover:text-white transition-all border border-white/5 shadow-2xl z-20"
                                                                >
                                                            ...
                                                            <div className="flex flex-col items-center justify-center h-full gap-3 opacity-20 group-hover/cell:opacity-100 transition-all duration-500 scale-90 group-hover/cell:scale-100">
                                                                <div className="w-14 h-14 rounded-3xl bg-white/5 flex items-center justify-center text-white/40 group-hover:text-brand-magenta group-hover:bg-brand-magenta/10 group-hover:border group-hover:border-brand-magenta/30 transition-all shadow-inner">
